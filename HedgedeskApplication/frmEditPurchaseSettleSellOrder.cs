...
{
    public partial class frmEditPurchaseSettleSellOrder : Form
    {
        ...
        DataTable dtSellOrdersFees = new DataTable();
        ...
        public frmEditPurchaseSettleSellOrder()
        {
            InitializeComponent();
        }
        ...
        private void LoadForm()
        {
            try
            {
                ...
                GlobalStore.FillSellOrderDataTable(mOrderNumber.ToString(), dtSellOrder);
                GlobalStore.FillSellOrderFeesDataTable(mOrderNumber.ToString(), dtSellOrdersFees);
                viewOrder = dtSellOrder.DefaultView;
                if (viewOrder.Count > 0)
                {
                    ...
                    this.dgvSellOrdersFees.AutoGenerateColumns = false;
                    this.dgvSellOrdersFees.DataSource = dtSellOrdersFees;
                    //BuildLabel();
                }
                else
                {
        ...
        private void dgvSellOrdersFees_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (this.dgvSellOrdersFees.Columns[e.ColumnIndex].Name != "DeleteSellFee") return;
            Point p = dgvSellOrdersFees.PointToClient(new Point(e.RowIndex, e.ColumnIndex));
            DataGridView.HitTestInfo info = dgvSellOrdersFees.HitTest(p.X, p.Y);

            if (info.RowIndex != -1 && info.ColumnIndex != -1)
            {
                DataGridViewCell cell = this.dgvSellOrdersFees[info.ColumnIndex, info.RowIndex];
            }

            Int32 iRowIndex = Convert.ToInt32(e.RowIndex);

            // Set as selected 
            dgvSellOrdersFees.Rows[iRowIndex].Selected = true;
               
            var feeID = Convert.ToInt32(dgvSellOrdersFees.Rows[iRowIndex].Cells["DeleteSellFee"].Value.ToString());
            Maintenance ord = new Maintenance();

            if(ord.DeleteFee(feeID, "Sell"))
            {
        ...
        private void dgvSellOrdersFees_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (isRowChanged == false)
            {
                mColumnIndex = dgvSellOrdersFees.CurrentCell.ColumnIndex;
                mRowIndex = dgvSellOrdersFees.CurrentCell.RowIndex;
            }
            isRowChanged = true;
        }

        private void dgvSellOrdersFees_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (isRowChanged)
            {
                try
                {
                    dgvSellOrdersFees.EndEdit();
                    string rIndex = dgvSellOrdersFees.Rows[e.RowIndex].Index.ToString();
                    ...
                    FeeTypeID = dgvSellOrdersFees.Rows[riIndex].Cells["FeeTypeID"].Value.ToString();
                    Amount = dgvSellOrdersFees.Rows[riIndex].Cells["Amount"].Value.ToString();


                    if (!clsPurchaseSettle.UpdateSellOrdersFees(OrderID, FeeTypeID, Amount, ref ReturnMessage))
                    {
        ...
        private void btnUpdateSellOrder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!clsPurchaseSettle.UpdatePurchaseSettleSellOrders(OrderID, txtCardNumber.Text, txtQty.Text, txtOrdType.Text, txtPrice.Text,
        ...
        private void dgvSellOrdersFees_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            if (isNotValidated)
                e.Cancel = true;
        }

        private void dgvSellOrdersFees_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btnMoveOrder_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Move " + txtOrderNumber.Text.ToString() + " to Buy Order table?", "Hedge Order", MessageBoxButtons.OKCancel) == DialogResult.Cancel)
                {
                    return;
                }
                else
                {
                    ...
                    if (!clsPurchaseSettle.MoveSellOrders(OrderID, ref ReturnMessage))
                    {
                        MessageBox.Show(ReturnMessage, "Hedge Orders");
        ...
        private void btnAddFee_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!clsPurchaseSettle.AddSellOrdersFees(OrderID, PSFeeTypeID, PSFeeAmount, ref ReturnMessage))
                {
                    MessageBox.Show(ReturnMessage, "Hedge Orders");
                ...
                {
                    dtSellOrdersFees.Clear();
                    GlobalStore.FillBuyOrderFeesDataTable(mOrderNumber.ToString(), dtSellOrdersFees);
                    this.dgvSellOrdersFees.AutoGenerateColumns = false;
                    this.dgvSellOrdersFees.DataSource = dtSellOrdersFees;
